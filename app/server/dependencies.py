...
def provide_select_in_str_filter[T](
    column: StringOrNone = Parameter(query="selectInField", default=None, required=False),
    values: StringOrNone = Parameter(query="selectInValues", default=None, required=False),
) -> CollectionFilter[T]:
    """Return type consumed by ``Repository.filter_in_collection()``.
...
def provide_search_filter(
    ...
    search_string: StringOrNone = Parameter(title="Field to search", query="searchString", default=None, required=False),
    ...
    return SearchFilter(field_name=field, value=search_string, ignore_case=ignore_case or False)  # type: ignore[arg-type]
...
def provide_filter_dependencies(
    ...
    select_in_str_filter: CollectionFilter = Dependency(skip_validation=True),
) -> list[FilterTypes]:
